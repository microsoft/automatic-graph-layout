...
namespace Microsoft.Msagl.Routing
{
	...
	public class SplineRouter : AlgorithmBase
	{
		...
		private IEnumerable<EdgeGeometry> edgeGeometriesEnumeration { get { return this._edges.Select(e => e.EdgeGeometry); } }
		public BundlingStatus BundlingResult { get; private set; } = BundlingStatus.None;
		public bool ContinueOnOverlaps { get; set; } = true;
		internal double MultiEdgesSeparation { get; set; } = 20;
		/// <summary>
		...
		/// </summary>
		public bool RouteMultiEdgesAsBundles { get; set; } = true;
		/// <summary>
		...
		/// </summary>
		public SplineRouter(GeometryGraph graph, EdgeRoutingSettings edgeRoutingSettings) : this(graph, edgeRoutingSettings.Padding, edgeRoutingSettings.PolylinePadding, edgeRoutingSettings.ConeAngle, edgeRoutingSettings.BundlingSettings)
		{ }

		/// <summary>
		/// Creates a spline group router for the given graph.
		/// </summary>
		public SplineRouter(GeometryGraph graph, double tightTightPadding, double loosePadding, double coneAngle) : this(graph, graph.Edges, tightTightPadding, loosePadding, coneAngle, null)
		{
		}
		...
		/// Creates a spline group router for the given graph
		/// </summary>
		public SplineRouter(GeometryGraph graph, double tightTightPadding, double loosePadding, double coneAngle, BundlingSettings bundlingSettings) : this(graph, graph.Edges, tightTightPadding, loosePadding, coneAngle, bundlingSettings)
		{
		}
		...
		void Initialize(IEnumerable<Shape> obstacles, double coneAngleValue)
		{
		...
		{
			if (!edgeGeometriesEnumeration.Any()) return;
			GetOrCreateRoot();
			RouteOnRoot();
			RemoveRoot();
		...
		void RouteOnRoot()
		{
			...
			if (OverlapsDetected && !ContinueOnOverlaps) return;
			BindLooseShapes();
		...
		*/
		void CalculatePortsToEnterableShapes()
		{
			...
			foreach (var portsToShape in portsToShapes)
			{
				...
				if (!(EdgesAttachedToPortAvoidTheNode(port))) set.Insert(portsToShape.Value);
		...
		void SetLoosePolylinesForAnywherePorts()
		{
			foreach (var shapesToTightLooseCouple in shapesToTightLooseCouples)
			{
				...
				foreach (var port in shape.Ports)
				{
					...
					if (aport != null) aport.LoosePolyline = (Polyline)shapesToTightLooseCouple.Value.LooseShape.BoundaryCurve;
					var clusterBoundaryPort = port as ClusterBoundaryPort;
					if (clusterBoundaryPort != null) clusterBoundaryPort.LoosePolyline = (Polyline)shapesToTightLooseCouple.Value.LooseShape.BoundaryCurve;
				}
			}
		}
		...
		void CalculateShapeToBoundaries(Shape shape)
		{
			...
			foreach (Shape child in shape.Children) CalculateShapeToBoundaries(child);

			var obstacleCalculator = new ShapeObstacleCalculator(shape, tightPadding, AdjustedLoosePadding, shapesToTightLooseCouples);
		...
		void RouteEdgesWithTheSamePassport(IGrouping<Set<Shape>, Edge> edgeGeometryGroup, InteractiveEdgeRouter interactiveEdgeRouter, Set<Shape> obstacleShapes)
		{
			...
			if (RouteMultiEdgesAsBundles)
			{
				...
				foreach (var edge in regularEdges) RouteEdge(interactiveEdgeRouter, edge);
				if (multiEdges != null)
				{
			...
			{
				foreach (var eg in edgeGeometryGroup) RouteEdge(interactiveEdgeRouter, eg);
			}
		}
		...
		{
			BundlingSettings bs = new BundlingSettings { InkImportance = 0.00001, EdgeSeparation = MultiEdgesSeparation };
			var mer = new MultiEdgeRouter(multiEdges, interactiveEdgeRouter, parents.SelectMany(p => p.Children).Select(s => s.BoundaryCurve), bs, MakeTransparentShapesOfEdgeGeometryAndGetTheShapes);
		...
		Set<Shape> GetObstaclesFromPassport(Set<Shape> passport)
		{
			...
			foreach (var shape in passport.Where(shape => !commonAncestors.Contains(shape))) queue.Enqueue(shape);
			...
			while (queue.Count > 0)
			{
				...
				foreach (var parent in a.Parents)
				{
					...
					{
						if (!allAncestors.Contains(sibling)) ret.Insert(sibling);
					}
		...
		Set<Shape> GetAllAncestors(Set<Shape> passport)
		{
			...
			foreach (var shape in passport) ret += ancestorSets[shape];
			return ret;
		}
		...
		Set<Shape> GetCommonAncestorsAbovePassport(Set<Shape> passport)
		{
			...
			foreach (var shape in passport.Skip(1)) ret *= ancestorSets[shape];
			return ret;
		}
		...
		Set<CdtEdge> FindCdtGates(Cdt cdt)
		{
			...
			foreach (var cdtSite in cdt.PointsToSites.Values)
			{
				foreach (var cdtEdge in cdtSite.Edges)
				{
					...
					if (cdtEdge.CwTriangle == null && cdtEdge.CcwTriangle == null) continue;
					...
					Shape aParent;
					Shape bParent;
					if (loosePolylinesToLooseParentShapeMap.TryGetValue(a, out aParent) && loosePolylinesToLooseParentShapeMap.TryGetValue(b, out bParent) && aParent == bParent)
					{
		...
		void GetEdgeEnterablePolylines(EdgeGeometry edgeGeometry, out Set<Polyline> looseEnterable, out Set<Polyline> tightEnterable)
		{
		...
		{
			return RectangleNode<Polyline>.CreateRectangleNodeOnEnumeration(shapesToTightLooseCouples.Values.Select(tl => new RectangleNode<Polyline>(tl.TightPolyline, tl.TightPolyline.BoundingBox)));
		}
		...
		void RouteEdgeGeometry(Edge edge, InteractiveEdgeRouter iRouter)
		{
			...
			if (!(edgeGeometry.SourcePort is HookUpAnywhereFromInsidePort)) addedEdges.AddRange(AddVisibilityEdgesFromPort(edgeGeometry.SourcePort));
			if (!(edgeGeometry.TargetPort is HookUpAnywhereFromInsidePort)) addedEdges.AddRange(AddVisibilityEdgesFromPort(edgeGeometry.TargetPort));
			...
			if (edgeGeometry.Curve == null) throw new NotImplementedException();


			foreach (var visibilityEdge in addedEdges) VisibilityGraph.RemoveEdge(visibilityEdge);
			...
			if (ReplaceEdgeByRails != null) ReplaceEdgeByRails(edge);
			//  SetTransparency(transparentShapes, false);
		}
		...
		IEnumerable<VisibilityEdge> AddVisibilityEdgesFromPort(Port port)
		{
			...
			if (port is CurvePort || !portsToShapes.TryGetValue(port, out portShape) || !shapesToTightLooseCouples.TryGetValue(portShape, out boundaryCouple))
			{
				return new VisibilityEdge[] { };
			}

			var portLoosePoly = boundaryCouple.LooseShape;
			return (from point in portLoosePoly.BoundaryCurve as Polyline where visGraph.FindEdge(port.Location, point) == null select visGraph.AddEdge(port.Location, point));
		}
		...
		List<Shape> MakeTransparentShapesOfEdgeGeometryAndGetTheShapes(EdgeGeometry edgeGeometry)
		{
			...
			foreach (var shape in portsToEnterableShapes[edgeGeometry.SourcePort]) transparentLooseShapes.Add(LooseShapeOfOriginalShape(shape));
			foreach (var shape in portsToEnterableShapes[edgeGeometry.TargetPort]) transparentLooseShapes.Add(LooseShapeOfOriginalShape(shape));
	 ...
	 visGraph.Edges.Select(
		...
		IEnumerable<Shape> GetTransparentShapes(Port sourcePort, Port targetPort, Shape sourceShape, Shape targetShape)
		{
			foreach (var s in ancestorSets[sourceShape]) yield return s;
			foreach (var s in ancestorSets[targetShape]) yield return s;
			...
			{
				if (IsAncestor(sourceShape, targetShape)) yield return sourceShape;
			}
			else
			{
				if (IsAncestor(targetShape, sourceShape)) yield return targetShape;
			}
		}

		static void SetTransparency(IEnumerable<Shape> shapes, bool v)
		{
			foreach (Shape shape in shapes) shape.IsTransparent = v;
		}
...
*/
		...
		*/
		...
		{
			return RectangleNode<Polyline>.CreateRectangleNodeOnEnumeration(loosePolys.Select(poly => new RectangleNode<Polyline>(poly, poly.BoundingBox)));
		}
		...
		RectangleNode<Polyline> CreateTightObstacleHierarachy(IEnumerable<Shape> obstacles)
		{
			...
			return RectangleNode<Polyline>.CreateRectangleNodeOnEnumeration(tightPolys.Select(tightPoly => new RectangleNode<Polyline>(tightPoly, tightPoly.BoundingBox)));
		}
		...
		void CalculateVisibilityGraph()
		{
			...
			foreach (var edgeGeometry in edgeGeometriesEnumeration)
			{
				if (!(edgeGeometry.SourcePort is HookUpAnywhereFromInsidePort || edgeGeometry.SourcePort is ClusterBoundaryPort)) setOfPortLocations.Insert(edgeGeometry.SourcePort.Location);
				if (!(edgeGeometry.TargetPort is HookUpAnywhereFromInsidePort || edgeGeometry.TargetPort is ClusterBoundaryPort)) setOfPortLocations.Insert(edgeGeometry.TargetPort.Location);
			}
			portRTree = new RTree<Point>(setOfPortLocations.Select(p => new KeyValuePair<Rectangle, Point>(new Rectangle(p), p)));
		...
		void FillVisibilityGraphUnderShape(Shape shape)
		{
			...
			foreach (Shape child in children) FillVisibilityGraphUnderShape(child);
			...
			foreach (VisibilityEdge edge in tmpVisGraph.Edges) TryToCreateNewEdgeAndSetIsPassable(edge, looseShape);
		...
		{
			var l = new List<DebugCurve>(tmpVisGraph.Edges.Select(e => new DebugCurve(100, 1, e.IsPassable != null && e.IsPassable() ? "green" : "black", new LineSegment(e.SourcePoint, e.TargetPoint))));
			if (obstacles != null) l.AddRange(obstacles.Select(p => new DebugCurve(100, 1, "brown", p)));
			if (greenCurves != null) l.AddRange(greenCurves.Select(p => new DebugCurve(100, 10, "navy", p)));
			if (redCurves != null) l.AddRange(redCurves.Select(p => new DebugCurve(100, 10, "red", p)));
		...
		void AddBoundaryEdgesToVisGraph(Polyline boundary)
		{
			...
			do
			{
				...
				if (pn == boundary.StartPoint) break;
				p = pn;
			}
			while (true);
		}
			...
			foreach (var point in portLocationsInQuestion)
			{
				switch (Curve.PointRelativeToCurveLocation(point, boundary))
				{
					...
					case PointLocation.Boundary:
						...
						if (polylinePoint != null) LineSweeper.InsertPointIntoPolylineAfter(boundary, polylinePoint, point);
		...
		static PolylinePoint FindPointOnPolylineToInsertAfter(Polyline boundary, Point point)
		{
			for (PolylinePoint p = boundary.StartPoint; ;)
			{
				...
				if (ApproximateComparer.Close(point, p.Point) || ApproximateComparer.Close(point, pn.Point)) return null; //the point is already inside
				double par;
				if (ApproximateComparer.Close(Point.DistToLineSegment(point, p.Point, pn.Point, out par), 0)) return p;
				p = pn;
				if (p == boundary.StartPoint) throw new InvalidOperationException();
			}
		}
		...
		void GetOrCreateRoot()
		{
			...
			root.UserData = "root";
#endif
			foreach (var rootShape in rootShapes) root.AddChild(rootShape);
		}
		...
		void RemoveRoot()
		{
			if (rootWasCreated)
			{
				foreach (var rootShape in rootShapes) rootShape.RemoveParent(root);
			}
		}
		...
		[SuppressMessage("Microsoft.Performance", "CA1811:AvoidUncalledPrivateCode")]
		static void Show(IEnumerable<EdgeGeometry> edgeGeometries, IEnumerable<Shape> listOfShapes)
		{
		...
		internal static Dictionary<Shape, Set<Shape>> GetAncestorSetsMap(IEnumerable<Shape> shapes)
		{
			...
			foreach (var child in shapes.Where(child => !ancSets.ContainsKey(child))) ancSets[child] = GetAncestorSet(child, ancSets);
			return ancSets;
		}
		...
		static Set<Shape> GetAncestorSet(Shape child, Dictionary<Shape, Set<Shape>> ancSets)
		{
			...
			foreach (var parent in child.Parents)
			{
				...
				ret += ancSets.TryGetValue(parent, out grandParents) ? grandParents : ancSets[parent] = GetAncestorSet(parent, ancSets);
			}
			return ret;
		}
