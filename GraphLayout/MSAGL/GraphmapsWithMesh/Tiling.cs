...
namespace Microsoft.Msagl.GraphmapsWithMesh
{
    ...
    public class Tiling
    {
        ...
        public void PlotAllEdges(WeightedPoint[] pt, Network g, int numPoints, int numOfLevels)
        {
            ...
            {
                _sNet = t.ComputeTree(VList, EList, DegList, NumOfnodes, _sNet, level);
            }

            for (int i = 1; i <= NumOfnodes; i++) VList[i].CId = 0;
            foreach (Vertex w in _sNet.V) w.CId = 1;

            Console.WriteLine("Compute the local neighborhood: Shortcut Mesh.");
            ComputeShortcutMesh(pt, numPoints);
            ComputeDetourAroundVertex(pt, numPoints);
                ...
                for (int j = i + 1; j <= numPoints; j++)
                {
                    if (g.M[i, j] == 1)
                    {
                        ...
                        foreach (VertexNeighbor e in dijkstra.Edgelist)
                        {
                            ...
                            _sNet.Segments.Add(new VertexNeighbor(e.A, e.Neighbor));
            ...
            for (int index = 1; index <= NumOfnodes; index++)
            {
                for (int neighb = 1; neighb <= DegList[index]; neighb++)
                {
                    if (EList[index, neighb].Used > 0) EList[index, neighb].Used = 1;
                }
            }
            ...
            for (int i = 1; i <= 1; i++)
            {
                for (int j = i + 1; j <= numPoints; j++)
                {
                    if (g.M[i, j] == 1)
                    {
                        ...
                        {
                            EList[e.A, e.Neighbor].Used++;
                            if (Source[i] == null) Source[i] = new ShortestPathEdgeList();
                            Source[i].Edgelist.Add(e);
                        }
                    }
                }
            }
        ...
        public void MsaglDetour(Dictionary<int, Node> idToNode)
        {
            ...
            nodeTree.Clear();
            for(int index = 0; index < NumOfnodes; index++)
                nodeTree.Add(new Rectangle(new Microsoft.Msagl.Core.Geometry.Point(VList[index].XLoc, VList[index].YLoc)),  index );
