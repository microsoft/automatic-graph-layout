...
namespace Microsoft.Msagl.UnitTests.Constraints
{
    ...
    public class ProjectionSolverTests : ProjectionSolverVerifier
    {
        ...
        [TestMethod]
        [Timeout(95 * 1000)]
        [Description("Test the simple Stopwatch implementation (because SilverLight currently doesn't have one).")]
        [TestCategory("NonRollingBuildTest")]
        public void SimpleStopwatchTest()
        {
            // The timeout is the max (3 * 10 passes at 3.1 seconds per pass plus 2 seconds overhead allowance),
            // but since we do Rand() for the number of seconds it should take on average half the max timeout.
            var sw = new Stopwatch();
            var ssw = new SimpleStopwatch();
            bool succeeded = true;
            ...
            for (int numReps = 0; numReps < 100; ++numReps)
            {
                ...
                {
                    WriteLine("Stopping: {0}; Resetting = {1}", (1 == pass) ? "true" : "false", (2 == pass) ? "true" : "false");
                    for (int ii = 0; ii < 10; ++ii)
                    {
                        sw.Start();
                        ssw.Start();
                        int sleepMs = rand.Next(3000) + 100; // .1 to 3.1 seconds.
                        WriteLine("SimpleStopwatch_Test Sleeping for {0} ms", sleepMs);
                        System.Threading.Thread.Sleep(sleepMs);

                        // The first pass tests "on-the-fly" elapsed retrieval, the second pass tests Stop()...
                        if (1 == pass)
                        {
                            sw.Stop();
                            ssw.Stop();
                        }
                        WriteLine("Iter {0}: Elapsed MS: sw = {1}, ssw = {2}", ii, sw.ElapsedMilliseconds, ssw.ElapsedMilliseconds);

                        // Allow a small percentage of variance to account for cumulative drift due to the time between calls.
                        if (Math.Abs(sw.ElapsedMilliseconds - ssw.ElapsedMilliseconds) > Math.Max(50, sw.ElapsedMilliseconds * 0.02))
                        {
                            WriteLine("Failure:  Elapsed MS: sw = {0}, ssw = {1}", sw.ElapsedMilliseconds, ssw.ElapsedMilliseconds);
                            succeeded = false;
                        }

                        // ... and the third pass tests Reset().
                        if (2 == pass)
                        {
                            sw.Reset();
                            ssw.Reset();
            ...
            Validate.IsTrue(succeeded, "Elapsed times were not equal");
        }
#endif // SILVERLIGHT
    }
}
