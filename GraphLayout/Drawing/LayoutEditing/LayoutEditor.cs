...
namespace Microsoft.Msagl.Drawing
{
    ...
    */
		...
		void AnalyzeLeftMouseButtonClick()
		{
			...
			if (obj != null)
			{
				...
				if (editableEdge != null)
				{
					...
					if (drawingEdge?.IsVisible ?? false)
					{
		...
		void ProcessRightClickOnSelectedEdge(MsaglMouseEventArgs e)
		{
			...
			cornerInfo = AnalyzeInsertOrDeletePolylineCorner(mouseRightButtonDownPoint, SelectedEdge.RadiusOfPolylineCorner);

			if (cornerInfo == null) return;

			e.Handled = true;

			var edgeRemoveCouple = new Tuple<string, VoidDelegate>("Remove edge", () => viewer.RemoveEdge(SelectedEdge, true));
			var menuSeparator = new Tuple<string, VoidDelegate>("-", () => { });
			var edgeCopyText = new Tuple<string, VoidDelegate>("Copy text", () => viewer.CopyEdgeText(SelectedEdge, true));
			var insertPolyLineCorner = new Tuple<string, VoidDelegate>("Insert polyline corner", InsertPolylineCorner);
			var deleteCorner = new Tuple<string, VoidDelegate>("Delete polyline corner", DeleteCorner);
			if (cornerInfo.Item2 == PolylineCornerType.PreviousCornerForInsertion) viewer.PopupMenus(insertPolyLineCorner, edgeRemoveCouple, menuSeparator, edgeCopyText);
			else if (cornerInfo.Item2 == PolylineCornerType.CornerToDelete) viewer.PopupMenus(deleteCorner, edgeRemoveCouple, menuSeparator, edgeCopyText);
		}
